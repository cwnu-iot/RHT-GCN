...
import time
import numpy as np
from CRHTA_ADGCN import CRHTA_ADGCN
import torch
...
rnn=CRHTA_ADGCN(num_class=21, num_point=16, num_person=1, in_channels=3,
...
def test(ep):
    ...
    if ep == 1:
        torch.save(rnn.state_dict(), 'CRHTA-ADGCN.mdl')
    with torch.no_grad():
        ...
        if ep == 49:
            with open("CRHTA-ADGCN.txt", "a+") as f:
                f.write('\n')
